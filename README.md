...
The current [DBIx::Connector](https://metacpan.org/pod/DBIx%3A%3AConnector) execution method name being called, which would either be
`run` or `txn`.  Since `svp` is not overridden, it would never be encountered.  If the
